...
namespace ABPEcommerce.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(ABPEcommerceConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(ABPEcommerceConsts.ProductIdentitySettingId, "Sản phẩm", ABPEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
