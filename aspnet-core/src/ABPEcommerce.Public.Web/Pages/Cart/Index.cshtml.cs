...
namespace ABPEcommerce.Public.Web.Pages.Cart
{
    public class IndexModel : PageModel
    {
        ...
        {
            var cart = HttpContext.Session.GetString(ContantsCacheKey.CacheKeys.Cart);
            ...
            if (!string.IsNullOrEmpty(action))
            {
                if (action == "add")
                {
                    ...
                    if (cart == null)
                    {
                        ...
                        HttpContext.Session.SetString(ContantsCacheKey.CacheKeys.Cart, JsonSerializer.Serialize(productCarts));
                    }
                    else
                    {
                        productCarts = JsonSerializer.Deserialize<Dictionary<string, CartItem>>(cart);
